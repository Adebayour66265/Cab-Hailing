...
const lockVehicle = async (req, res) => {
  try {
    ...
    if (!vehicle) return res.status(404).json({ message: "Vehicle not found" });
    const response = await axios.post("http://hardware-api.com/lock", {
...
const unlockVehicle = async (req, res) => {
  try {
    ...
    const vehicle = await Vehicle.findOne({ qrCode: qrCodeData });
    if (!vehicle) return res.status(404).json({ message: "Vehicle not found" });
    const response = await axios.post("http://hardware-api.com/unlock", {
      vehicleId: vehicle.id,
    });
    ...
    res
      .status(500)
      .json({ message: "Failed to unlock vehicle" + error.message });
  }
};

// API endpoint to submit ride requests
const rideRequest = async (req, res) => {
  const { user, location } = req.body;
  const newRequest = new Rental({
    user,
