...
const addCar = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Vehicle created successfully", vehicle: vehicle });
  } catch (error) {
    console.log(error);
...
// API endpoint for drivers to update availability status
const driverAvailability = async (req, res) => {
  const { name, location, isAvailable } = req.body;
  const newDriver = new Vehicle({
    name,
  ...
  try {
    ...
    res.json({ message: "Driver availability updated successfully." });
  } catch (error) {
